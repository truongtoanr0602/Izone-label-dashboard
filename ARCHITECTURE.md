...
|---|---|---|
| `teachers` | `08_GiaoVien` | Master GV: `teacher_id`, `teacher_name`, `teacher_email` (unique), `teacher_phone`, `khoi_id` (mặc định 2), `role` (`teacher`/`lead`/`admin`). `khoi_id` ánh xạ một-một với `classes.course_id`: 1 = Khối 03, 2 = Khối 3-4, 3 = Khối 4-5 |
| `teacher_khoi_assignments` | *(không có trong 9 sheet gốc)* | Phân quyền nhiều-nhiều giữa tài khoản và khối. Khóa chính `(teacher_id, khoi_id)`; `is_primary` chọn khối mặc định. `teachers.khoi_id` được giữ tạm làm fallback trong giai đoạn chuyển tiếp và được backfill bởi migration `013_lead_khoi_assignments.sql`. |
...
| `dashboards` | `GET /api/v1/lead-dashboard` | Namespace `/api/v1/*` — khác mọi endpoint khác ở trên (`/api/...`, không version, xem §6 mục namespace không nhất quán). Query `period=YYYY-MM` (khuyến nghị) hoặc `from`/`to` (tối đa 90 ngày) — hai kiểu loại trừ lẫn nhau. Hỗ trợ `courseId`/`khoiId` 1/2/3; với `role=lead`, backend chỉ chấp nhận `khoiId` nằm trong `teacher_khoi_assignments`, nếu không trả 403. Query dữ liệu theo `classes.course_id`, không theo khối của GVCN. `role=teacher` bị chặn 403. Trả KPI/xu hướng tuần/nhãn đã tính sẵn — xem "Dashboards module" dưới đây |
...
- **Tập lớp của Lead phụ thuộc kỳ báo cáo, không phụ thuộc riêng trạng thái hiện tại.** Một lớp chỉ vào `GET /api/v1/lead-dashboard` khi có ít nhất một `class_daily_snapshots.snapshot_date` trong toàn bộ tháng `period` được chọn và trạng thái hiện tại thuộc `on_going`/`completed`; `pending`/`cancelled` bị loại. Cùng predicate `EXISTS` này phải xuất hiện ở cả 7 query có join `classes` (`classRows`, `snapshotRows`, `studentMetricRows`, `transitionRows`, `coverageStudentRows`, `coverageTestRows`, `contactLogRows`) để Master Table, KPI, pass và contact coverage không lệch tập lớp. Cutoff metric vẫn dùng `reportAsOf`/`currentAsOf`, nên snapshot ở cuối tháng chỉ xác nhận tư cách lớp chứ không làm lộ số liệu tương lai.
- **Snapshot Quality Resolver** (`snapshot-quality.ts`, hàm `resolveClassObservation`): loại bỏ dòng có ngày tương lai; chọn roster từ snapshot mới nhất ≤ `asOf`; `progress.totalSessions` lấy `classes.total_sessions` làm nguồn chuẩn, còn `completedSessions` ưu tiên số buổi lũy kế lớn nhất quan sát được từ `attendance_total`/`homework_total` trong `student_daily_records`, giới hạn ở 100%. Chỉ khi hoàn toàn không có bộ đếm học viên mới fallback sang snapshot và gắn `PROGRESS_FALLBACK_USED`; snapshot lệch tử/mẫu số được gắn `PROGRESS_SNAPSHOT_MISMATCH`/`SNAPSHOT_TOTAL_SESSIONS_MISMATCH`. `attendance`/`homework` nhận dòng mới nhất **có mẫu** (`sampleSize > 0` và giá trị khác `null`), nếu dòng mới nhất không có mẫu thì lùi về dòng cũ hơn gần nhất có (`fallbackUsed: true`); tỷ lệ pass chia cho `testedStudents` (số HV đã thi), không phải tổng sĩ số, `0%` vẫn hợp lệ nếu có mẫu.
- **Repair tiến độ snapshot:** migration `006_repair_snapshot_completed_sessions.sql` (2026-08-18) sửa `completed_sessions` cho dữ liệu cũ. Sau khi course 1/3 được nạp, production lại xuất hiện snapshot có `completed_sessions=0..1` và `total_sessions=28` mặc định; migration `012_repair_multicourse_snapshot_progress.sql` audit rồi chuẩn hóa cả hai cột cho course 1/2/3 từ bộ đếm học viên và `classes.total_sessions`. Cả hai migration đều không chạm trực tiếp generated column `progress_pct`. Pipeline ingestion bên ngoài vẫn phải ghi cùng công thức để lỗi không tái diễn; runtime resolver bảo vệ UI trong thời gian pipeline chưa được sửa.
- **Không còn ngưỡng độ phủ tối thiểu ở tầng snapshot.** `coveragePct` (`sampleSize/recordCount`) chỉ để hiển thị (tooltip biểu đồ, note của card: `228/264 HV`) — Lead tự đánh giá độ tin cậy thay vì bị hệ thống âm thầm loại lớp khỏi mẫu số. `LOW_COVERAGE_WARNING_PCT = 80` chỉ gắn cảnh báo `LOW_ATTENDANCE_COVERAGE`/`LOW_HOMEWORK_COVERAGE` lên `dataQuality.warnings[]`. Lớp chỉ bị đánh `dataQuality.status: 'insufficient'` khi `attendance`/`homework` hoàn toàn không có dòng nào có mẫu (không phải khi mẫu mỏng) — status còn lại (`complete`/`fallback`) và toàn bộ `warnings[]` được trả thẳng ra API.
...
JWT (`@nestjs/jwt`), secret đọc từ `process.env.JWT_SECRET`, **có fallback hardcode `'izone-secret-key-12345'` ngay trong `auth.module.ts` nếu thiếu env** — rủi ro bảo mật thật nếu quên set env khi deploy. Hết hạn sau 7 ngày. `teachers` chính là bảng user — không có bảng `User` riêng. Mỗi lần validate token, backend đọc lại `teacher_khoi_assignments` và trả `khoiIds`, `defaultKhoiId`, `khoiScopes` qua `/api/me`; vì quyền không cache trong JWT nên Lead đổi khối hoặc được cấp thêm quyền mà không cần đăng nhập lại. `classIds` của giáo viên cũng được tính lại mỗi lần validate token.
