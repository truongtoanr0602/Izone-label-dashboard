...
- **Repair tiến độ snapshot 2026-08-18:** migration `006_repair_snapshot_completed_sessions.sql` đã sửa các snapshot có `completed_sessions` sai bằng số đếm lũy kế lớn nhất trong `student_daily_records` của cùng lớp đến ngày snapshot, giới hạn bởi `classes.total_sessions`. Migration lưu giá trị cũ trong `class_snapshot_progress_repair_20260817`, không chạm snapshot không có bằng chứng và có thể chạy preview lại để phát hiện sai lệch mới. Repo không chứa writer production đã tạo các dòng cùng `scraped_at = 2026-08-17T10:13:23.426Z`; `system_logs` cũng không có entry tương ứng, nên pipeline ingestion/backfill bên ngoài vẫn là owner phải áp dụng cùng công thức để lỗi không tái diễn.
