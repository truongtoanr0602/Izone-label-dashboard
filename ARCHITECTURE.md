...
⚠️ **Lỗ hổng này VẪN ĐANG MỞ trên production — migration 008 chưa được chạy.** Dispatch của Task 1 (nhánh `fix/lead-dashboard-data-fidelity`) cố tình cấm áp dụng: "Do not apply migration 008 to the production database... Applying it is a deployment step outside this task." Kiểm tra trực tiếp trên `izone_dashboard` prod ngày 2026-08-13 (`pg_indexes` + `pg_constraint` trên `izone.student_daily_records`) xác nhận: constraint/index duy nhất liên quan tới `snapshot_stage` là `uq_student_stage`; **không có `uq_student_record_live` nào tồn tại** (chỉ có thêm `student_daily_records_pkey` và các FK/CHECK không liên quan). Nói cách khác: dòng live hiện **không có bất kỳ ràng buộc unique nào** chặn trùng lặp — nếu pipeline scrape chạy 2 lần trong một ngày cho cùng một HV, dòng live sẽ nhân đôi âm thầm và không có cột nào để lần ra, đúng như rủi ro cảnh báo ở `docs/superpowers/specs/2026-08-12-lead-dashboard-data-fidelity-design.md` §2.5. **Để đóng lỗ hổng này, ai đó phải chủ động chạy migration 008 lên prod** (`psql` hoặc cơ chế migration hiện có — xem §1 phần Provisioning) như một bước deploy riêng; đừng coi file migration đã commit git là bằng chứng nó đã chạy.
...
9. ~~**File migration mồ côi trong working tree.**~~ **Lỗi thời — sửa 2026-08-13, nhận định cũ đây sai với hiện trạng, nhưng rủi ro thay thế nó VẪN CÒN MỞ.** `006`/`007` đã commit git và đã chạy trên prod (xác nhận qua `pg_indexes`/`pg_constraint`: `idx_student_daily_stage`, `uq_student_stage`, `ck_student_daily_snapshot_stage` đều tồn tại) — toàn bộ module `dashboards` giờ phụ thuộc trực tiếp vào cột `snapshot_stage` (xem mục "Hai hạt trộn trong `student_daily_records`" cuối §1). **Nhưng `008` (`uq_student_record_live`) mới chỉ commit git, CHƯA được chạy trên prod** — dispatch của Task 1 cố tình cấm áp dụng nó ("Applying it is a deployment step outside this task"). Kết quả: dòng backfill (`snapshot_stage = 1..8`) có ràng buộc unique bảo vệ (`uq_student_stage`), nhưng dòng live (`snapshot_stage IS NULL`) thì **không** — ràng buộc 1-dòng-mỗi-hạt vẫn thiếu đúng nửa quan trọng hơn (dữ liệu live là thứ mọi KPI Lead đọc). Đây là rủi ro nghiệp vụ đang mở, không phải đã đóng: cần một bước deploy riêng (chạy `008_restore_live_record_uniqueness.sql` lên prod) để đóng nó thật sự — xem cảnh báo chi tiết ở §1.
