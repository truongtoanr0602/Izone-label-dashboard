...
import { ShieldCheck, HeartPulse, LayoutDashboard, Users, Award, X } from 'lucide-react';
...
export default function App() {
  ...
  // Navigation & Tabs
  const [activeTab, setActiveTab] = useState<'lead' | 'teacher' | 'review'>('teacher');
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans selection:bg-[#DB0829]/30 selection:text-white">
      {/* Top Header */}
      <Header
        classes={classes}
      ...
      <div className="flex-1 flex max-w-[1600px] w-full mx-auto relative overflow-hidden xl:overflow-visible">
        ...
        <aside className={`fixed inset-y-0 left-0 z-[60] w-64 transform transition-transform duration-300 xl:relative xl:translate-x-0 flex flex-col border-r border-slate-800/80 bg-slate-900 xl:bg-slate-900/40 p-5 space-y-6 ${isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}>
          {/* Mobile Header in Sidebar */}
          <div className="flex items-center justify-between xl:hidden mb-2">
            <span className="font-extrabold text-slate-100 text-lg">Menu</span>
            <button 
              onClick={() => setIsMobileMenuOpen(false)}
          ...
          <div className="glass-panel rounded-2xl p-4 border border-slate-800 bg-gradient-to-b from-slate-900 to-slate-950 space-y-3">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/60 p-5 rounded-2xl border border-slate-800/80">
                <div className="space-y-1">
                  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold bg-[#DB0829]/20 text-[#DB0829] border border-[#DB0829]/30 w-fit">
                  ...
                  <p className="text-xs text-slate-400 flex flex-wrap items-center gap-4">
                    <span>GVCN: <b className="text-slate-200">{selectedClass.teacher.fullName}</b></span>
                    <span>•</span>
                    <span>Lịch học: <b className="text-slate-200">{selectedClass.schedule}</b></span>
                    <span>•</span>
                    <span>Sĩ số: <b className="text-emerald-400 font-mono">{selectedClass.studentCounts.active} Active</b> / {selectedClass.studentCounts.totalEnrolled} Tổng</span>
                  </p>
                </div>
